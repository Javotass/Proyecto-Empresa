...
class ModelEvaluator:
    """Evaluacion de modelos de deteccion de anomalias"""
    ...
    def evaluate(self, y_true, y_pred, y_scores=None):
        ...
        print("\nMatriz de Confusion:")
        print(f"                Predicho Normal  Predicho Anomalia")
        print(f"Real Normal          {cm[0][0]:<15}  {cm[0][1]}")
        print(f"Real Anomalia        {cm[1][0]:<15}  {cm[1][1]}")
        
        # Metricas adicionales
        if y_scores is not None and len(np.unique(y_true)) > 1:
            auc = roc_auc_score(y_true, y_scores)
            print(f"\nArea bajo la curva ROC: {auc:.4f}")
    ...
    def analyze_anomalies(self, df, anomaly_scores, top_n=10):
        """Analiza las principales anomalias detectadas"""
        df_analysis = df.copy()
        ...
        print(f"\n=== TOP {top_n} TRANSACCIONES MAS ANOMALAS ===\n")
